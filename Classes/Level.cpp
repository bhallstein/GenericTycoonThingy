...
Level::Level(W::Window *_win, std::string levelpath) : win(_win)
{
	...
	W::Callback *c = new W::Callback(&Level::scrollEvent, this);
	...
	// Key subscriptions
	c = new W::Callback(&Level::keyEvent, this);
	eh.subscribeToKey(W::KeyCode::K_ESC, c);
	eh.subscribeToKey(W::KeyCode::K_Q, c);
	eh.subscribeToKey(W::KeyCode::K_C, c);
	eh.subscribeToKey(W::KeyCode::K_S, c);
	eh.subscribeToKey(W::KeyCode::K_H, c);
...
//		W::position p;
//		SpawnPoint *sp = spawnPoints[W::randUpTo((int)spawnPoints.size())];
//		sp->getCoords(&p);
//		
//		int q = W::randUpTo(3);
...
//	}
	...
	// Update TLOs
	for (std::vector<Unit*>::iterator it = units.begin(); it < units.end(); it++) (*it)->update();
	for (std::vector<Unit*>::iterator it = staff.begin(); it < staff.end(); it++) (*it)->update();
	for (std::vector<Building*>::iterator it = buildings.begin(); it < buildings.end(); it++) (*it)->update();
	for (std::vector<Behaviour*>::iterator it = behaviours.begin(); it < behaviours.end(); it++) (*it)->update();
...
void Level::keyEvent(W::Event *ev) {
	...
	else if (ev->key == W::KeyCode::K_ESC) { /*theW->playSound("Battle control terminated.wav");*/ W::popState(W::EmptyReturny); }
...
void Level::openHelpView() {
	...
	helpView = new GTTHelpView(win, &eh);
	addView(helpView);
...
void LevelView::draw() {
	...
	drawRect(0, 0, plan[0].sz.width, plan[0].sz.height, BLACK);
...
void LevelView::processMouseEvent(W::Event *ev) {
	...
	ev->pos.x = ev->pos.x / gridsize;
	ev->pos.y = ev->pos.y / gridsize;
...
void UIBarView::draw() {
	drawRect(0, 0, plan[0].sz.width, plan[0].sz.height, W::Colour::BLACK);
...
GTTHelpView::GTTHelpView(W::Window *_win, W::EventHandler *_eh) :
	UIView(
		new W::Positioner(W::TOP_LEFT, W::PFIXED, W::PFIXED, 140, 77, 520, 400),
		_win,
		_eh,
		W::ALLOW_DRAG
	)
